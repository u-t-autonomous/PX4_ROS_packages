...
void minbleicoptimize(minbleicstate &state,
    std::function<void(const real_1d_array& , double& , void*)> func,
    std::function<void(const real_1d_array& , double , void*)> rep = NULL,
    //void (*func)(const real_1d_array &x, double &func, void *ptr),
    //void  (*rep)(const real_1d_array &x, double func, void *ptr) = NULL,
    void *ptr = NULL);
void minbleicoptimize(minbleicstate &state,
    std::function<void(const real_1d_array& , double& , real_1d_array& , void*)> grad,
    std::function<void(const real_1d_array& , double , void*)> rep = NULL,
    //void (*grad)(const real_1d_array &x, double &func, real_1d_array &grad, void *ptr),
    //void  (*rep)(const real_1d_array &x, double func, void *ptr) = NULL,
    void *ptr = NULL);
...
* this algorithm is fast enough for large-scale problems with small amount
  of general linear constraints (say, up to 50),  but  it  is  inefficient
  for problems with several hundreds of  constraints.  Iteration  cost  is
  roughly quadratic w.r.t. constraint count.
  Furthermore, it can not efficiently handle sparse constraints (they  are
  converted to dense format prior to solution).
  Thus, if you have large and/or sparse constraint matrix  and  convex  QP
  problem, Dense-AUL-QP solver may be better solution.
...
* supports dense and sparse QP problems; although it uses  dense  Cholesky
  to build preconditioner, it still works faster for sparse problems.
...
* because dense Cholesky driver is used, this algorithm has O(N^2)  memory
  ...
  linear constraints.
* its precision is lower than that of BLEIC-QP and QuickQP. It is hard  to
  find solution with more than 6 digits of precision.
