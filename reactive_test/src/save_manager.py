...
class QuadControl (Thread):
	...
	def run(self):
		...
		while not rospy.is_shutdown():
			self.m_lock.acquire()
			pva_to_send = self.current_path[self.current_ind]
			self.current_ind += 1
			self.m_lock.release()
...
if __name__ == "__main__":
	...
	for elem in possible_target:
		if elem == possible_target[0]:
			continue
		publish_user_setpoint(grid.state2vicon(elem) , [0.0,1.0,0.0,1.0] , resolution)
		occupancy_wait.sleep()
	...
	agent_controller = tulip_controller.AgentController(initial_state,filename, X_NUMBER_TILE , Y_NUMBER_TILE ,waypoints_state , "Quad9" , possible_target, static_obstacles)
	agent_controller.getController()
	...
	for elem in range(occupancy_grid.info.width * occupancy_grid.info.height):
		if occupancy_grid.data[elem] == 100:
			...
			#publish_user_setpoint(grid.state2vicon(elem_y + elem_x * occupancy_grid.info.height), cube_size=occupancy_grid.info.resolution)
			#occupancy_wait.sleep()

	#raw_input('Clean the red static obstacles : ')
	#Extend static obstacles with a certain margin
	margin_size = 1
	new_static_obstacles = set()
	for elem in static_obstacles:
	...
	#Publish extra security points
	for elem in new_static_obstacles:
		#publish_user_setpoint(grid.state2vicon(elem) , [1.0 , 0.0 , 0.0 , 1.0] , occupancy_grid.info.resolution)
		occupancy_wait.sleep()
	...
	for elem in range(number_possible_target):
		...
		publish_user_setpoint(grid.state2vicon(grid.vicon2state(last_clicked_point)) ,[0.0,1.0,0.0,1.0], occupancy_grid.info.resolution)

	#instanciate tulip controller and create the controller
	name = raw_input("Controller name : ")
	raw_input('Click on the initial position : ')
	agent_controller = tulip_controller.AgentController(grid.vicon2state(last_clicked_point),name, X_NUMBER_TILE , Y_NUMBER_TILE ,waypoints_state , quad_name , possible_target, static_obstacles)
	#agent_controller.createController()
	...
	state_dict = list()
	state_dict.append(controller.move(next_target_state)[quad_name])
	next_target_state = 0
	for elem in range(precompute_path_step):
		next_moves = controller.move(next_target_state)
		state_dict.append(next_moves[quad_name])
	size_solution = 0
	pos_state = from_state_to_pva(state_dict , grid)
	...
	while not rospy.is_shutdown():
		...
		if ( (next_target_state == len(possible_target))):
			...
			state_dict.append(controller.move(len(possible_target))[quad_name])
