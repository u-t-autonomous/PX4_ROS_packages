...
#fixed_obstacle = [55, 42, 75, 27, 34] #10
fixed_obstacle = [73, 55, 42, 75, 27, 34]
...
for iter1 in sys_target_point:
	...
	for state in positions:
		if not state in fixed_obstacle:
...
for iter1 in valid_env_list:
	...
	# next_state_all.extend(get_point_strict_dist(iter1,margin+env_size+sys_size-2))
	next_state = getPointFromDist(iter1,gen_enabled_dir(enabled_dir_4,margin+env_size+sys_size,strict_dist=True))
...
#print ctrl
#print specs.pretty()
dumpsmach.write_python_case("followMeController_10_2_0_0_obs.py", ctrl, classname="FollowMeCtrl_10_obs")
