...
import static_env_controller as tulip_controller
...
class Grid:
	...
	def vicon2state(self, position):
		...
		try:
			...
			print "x position %r is out of bounds! It should be at least %r and at most %r. Replacing it with %r instead. " % (old_val, self.base.x, self.maximum.x, position.x)
		...
		try:
			...
			if(position.y < self.base.y):
				position.y = self.base.y
			if(position.y > self.maximum.y):
				position.y = self.maximum.y
			print "y position %r is out of bounds! It should be at least %r and at most %r. Replacing it with %r instead. " % (old_val, self.base.y, self.maximum.y, position.y)
...
class Send_Traj(Thread):
	def __init__(self , traj_pub , traj_freq ):
...
if __name__ == "__main__":
	...
	set_point_pub2 = rospy.Publisher('/Quad10/qcontrol/pva_setpoint', PoseStamped  , queue_size=10)
	...
	agent_controller = tulip_controller.AgentController(name , X_NUMBER_TILE , Y_NUMBER_TILE ,sys_specs ,static_obstacles , 0 ,0)
	agent_controller.create_controller()

	#Get all the state to achieve the mission planner
	state_dict = agent_controller.get_trajectory()
	#state_dict = agent_controller.get_trajectory_after(60)
	# for sys_name in state_dict:
	# 	for elem in state_dict[sys_name]:
	# 		publish_user_setpoint(grid.state2vicon(elem))
	#print state_dict

	#Transform trajectory to position 
	duration = float(raw_input('Trajectory total duration in second : '))
	freq_sol = 20
	final_solution = dict()
	size_solution = 0
	#first_env = True
	...
	for sys_name , target_state in state_dict.items():
		...
		res_traj = generate_traj([elem.x for elem in pos_state ],[elem.y for elem in pos_state ],[(i*duration)/(len(pos_state)-1) for i in range(len(pos_state))],freq=freq_sol)
		#res_traj = generate_traj([elem.x for elem in pos_state ],[elem.y for elem in pos_state ],[0 , duration],freq=freq_sol)
		send_waypoint(res_traj , publisher_dict[sys_name][1])
		final_solution[sys_name] = res_traj.pva
		size_solution = len(res_traj.pva)
		print size_solution , len(pos_state)

	#Some takeoff procedure and going to the initial state

	#Now send all the path with a good timing based on minimum snap return strategie
	raw_input("Start trajectory now : ") 
	wait_rate = rospy.Rate(freq_sol)
	for elem in range(size_solution):
		for sys_name , pva_sys in final_solution.items():
			if sys_name == "Quad9":
				pva_callback(pva_sys[elem],set_point_pub)
			else:
				pva_callback(pva_sys[elem],set_point_pub2)
