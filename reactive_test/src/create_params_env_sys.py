...
def get_information_quad(ind_quad):
	...
	for elem in range(number_setpoint):
		...
		publish_user_setpoint(grid.state2vicon(grid.vicon2state(last_clicked_point)) , m_color=[1.0/(ind_quad+1) , 1.0/(ind_quad+1) , 0.0 , 1.0] ,cube_size=resolution)
...
if __name__ == "__main__":
	...
	if use_occupancy == 1:
		...
		base = Point()
		base.x = float(raw_input('Base lower right x coord : '))
		base.y = float(raw_input('Base lower right y coord : '))
		base.z = Z_LEVEL
	...
	raw_input('Enter here infrmation on sys :')
	...
	raw_input("Click on starting point for sys: ")
	initial_state = grid.vicon2state(last_clicked_point)

	raw_input('Enter here infrmation on env :')
	(quad_name_env , waypoints_env) = get_information_quad(1)
	waypoints_state_env = grid.vicon2state_list(waypoints_env)

	number_possible_target = int(raw_input('Number of possible target : '))
	possible_target_env = list()
	#Add first point in possible target
	if len(waypoints_env) > 0:
		possible_target_env.append(waypoints_state_env[0])
	for elem in range(number_possible_target):
		raw_input(' Click on a target cell/Point and then press enter !')
		possible_target_env.append(grid.vicon2state(last_clicked_point))
		publish_user_setpoint(grid.state2vicon(grid.vicon2state(last_clicked_point)) ,[0.0,1.0,1.0,1.0], resolution)

	raw_input("Click on starting point for sys: ")
	initial_state_env = grid.vicon2state(last_clicked_point)
	...
	settings.write(str(initial_state)+';'+str(initial_state_env) +'\n')
	settings.write(";".join([str(state) for state in waypoints_state])+'\n')
	settings.write(";".join([str(state) for state in waypoints_state_env])+'\n')
	settings.write(";".join([str(state) for state in possible_target])+'\n')
	settings.write(";".join([str(state) for state in possible_target_env])+'\n')
