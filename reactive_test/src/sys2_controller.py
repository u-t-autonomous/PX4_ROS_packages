...
def set_stage(nb_stage, list_position1 , list_position2):
	plan = set()
	for stage in range(nb_stage):
		plan |= {('((stage = {stage}) && !((loc1 = {position}) && (loc2 = {position2}))) -> X (stage = {stage})'.format(stage=stage,position=list_position1[stage],position2=list_position2[stage]))}
		plan |= {('((stage = {stage}) && (loc1 = {position}) && (loc2 = {position2})) -> X (stage = {stage_1})'.format(stage=stage,position=list_position1[stage],position2=list_position2[stage],stage_1=(stage+1)))}
	return plan
...
sys_prog |= {'stage = '+str(nb_stage)}
#sys_prog |= {'stage1 = '+str(nb_stage-1)}
#sys_prog |= {'stage1 = '+str(nb_stage-1)}

sys_safe |= set_stage(nb_stage, list_pos_quad1 , list_pos_quad2)
...
for iter1 in range(dim * dim):
	...
	sys_safe |= {'! ( (loc1 = '+str(iter1) +') && (loc2 = '+str(iter1)+') )'}
