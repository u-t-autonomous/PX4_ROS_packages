...
def solve_problem(init_state , file_name, row_number , col_number , list_target , sys_name , sup_target_list , list_obstacles):
	...
	for elem in range(1,row_number-1):
		env_safe |= {'({env_name} = {state}) -> X ( ({env_name} = {state}) || ({env_name} = {state_plus}) || ({env_name} = {state_moins}))'.format(env_name='Quad8' , state=elem , state_moins=elem-1 , state_plus=elem+1)}
	env_safe |= {'({env_name} = 0) -> X( ({env_name} = 0) || ({env_name} = 1) )'.format(env_name='Quad8')}
	env_safe |= {'({env_name} = {last_elem}) -> X( ({env_name} = {last_elem}) || ({env_name} = {last_elem_moins}) )'.format(env_name='Quad8',last_elem_moins=row_number-2,last_elem=row_number-1)}
	...
	#System prog
	sys_prog = set()
	sys_prog |= {"(stage_{sys_name} = {last_value})".format(sys_name=sys_name,last_value=len(list_target),last_elem=row_number-1)}
	#sys_prog |= {"(stage_{sys_name} = {last_value}) -> X (Quad8 = 0) ".format(sys_name=sys_name,last_value=len(list_target))}
	#sys_prog |= {"Quad8 = 0"}
	#sys_prog |= {"Quad8 = "+str(row_number-1)}
	...
	# 	sys_safe |= {'! ( (Quad8 = {value}) && ({sys_name} = {value_val}))'.format(value=elem , value_val= elem *(row_number + 1) , sys_name=sys_name)}
	for elem in range(row_number):
		sys_safe |= {'( (Quad8 = {value}) -> !  (({sys_name} = {value_val}) )) '.format(sys_name=sys_name,value=elem,value_val=elem*(row_number+1))}
...
if __name__ == "__main__":
	...
	list_target = [10]
	sys_name = 'Quad9'
	sup_target_list = [14 , 21, 3 , 6]
	list_obstacles =[8,16]

	agent = AgentController(5,file_name , row_number , col_number , list_target , sys_name , sup_target_list , list_obstacles)
	agent.createController()
	controller = agent.getController()

	for elem in (range(50)):
		target = int(raw_input('target ind : '))
		env = int(raw_input('env_input : '))
		print controller.move( env , target)
