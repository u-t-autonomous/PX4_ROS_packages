...
def handle_static_obstacles(list_obstacles ,  sys_name , sys_name2):
	set_obstacle =  set()
	for obstacles in list_obstacles:
		set_obstacle |= {'! ({sys_name} = {obstacle_state})'.format(sys_name=sys_name , obstacle_state=obstacles)}
		set_obstacle |= {'! ({sys_name} = {obstacle_state})'.format(sys_name=sys_name2 , obstacle_state=obstacles)}
	return set_obstacle

def allow_moves(row_number , col_number , sys_name , sys_name2 , list_obstacles):
	set_moves = set()
	...
	for elem in possibilities:
		...
		set_moves |= {'({sys_name}={state}) -> X (({sys_name} = {state}) || ({list_valid_neighbor}))'.format(sys_name=sys_name2 , state=elem , list_valid_neighbor= get_spec_list_neighbor(sys_name,valid_state))}
	return set_moves
...
def handle_target_point(row_number , col_number , list_target , sys_name , sup_target_list):
	...
	for stage in range(len(list_target)):
		set_target |= {'(stage_{sys_name} = {stage}) && (! ({sys_name} = {position})) && (follow_{sys_name} = 1) -> X ((stage_{sys_name} = {stage}) )'.format(sys_name = sys_name , stage = stage, position = list_target[stage],target=len(sup_target_list))}
		set_target |= {'(stage_{sys_name} = {stage}) && ({sys_name} = {position}) && (follow_{sys_name} = 1) -> X ((stage_{sys_name} = {stage_1}) )'.format(sys_name = sys_name , stage = stage, position = list_target[stage], stage_1 = (stage+1),target=len(sup_target_list))}
		#set_target |= {'(stage_{sys_name} = {stage}) && follow_target = '}
		for target in range(len(sup_target_list)):
			set_target |= {'(follow_{sys_name} = 0) && (stage_{sys_name} = {stage}) && (! ({sys_name} = {position})) && (target_{sys_name} = {target}) -> X ((follow_{sys_name} = 0) && (stage_{sys_name} = {stage})) '.format(sys_name=sys_name,stage = stage,last_target=len(sup_target_list),target=target,position=sup_target_list[target])}
			set_target |= {'(follow_{sys_name} = 0) && (stage_{sys_name} = {stage}) &&  ({sys_name} = {position}) && (target_{sys_name} = {target})  -> X (((!(target_{sys_name}={last_target})) -> (follow_{sys_name} = 1)) && (stage_{sys_name} = {stage}))'.format(sys_name=sys_name,stage = stage,target=target,last_target=len(sup_target_list),position=sup_target_list[target])}

	for target in range(len(sup_target_list)):
	 	set_target |= {'(stage_{sys_name} = {stage}) && (! ({sys_name} = {position})) && (target_{sys_name} = {target}) -> X ((follow_{sys_name} = 0))'.format(sys_name=sys_name,stage = len(list_target),target=target,position=sup_target_list[target])}
	 	set_target |= {'(stage_{sys_name} = {stage}) &&  ({sys_name} = {position}) && (target_{sys_name} = {target}) -> X (((!(target_{sys_name}={last_target})) -> (follow_{sys_name} = 1)))'.format(sys_name=sys_name,stage = len(list_target),target=target,last_target=len(sup_target_list),position=sup_target_list[target])}
...
def solve_problem(init_state , init_state2 , file_name, row_number , col_number , list_target , list_target2 , sys_name , sys_name2, sup_target_list , sup_target_list2, list_obstacles):
	print row_number
	print col_number
	print sys_name
	...
	env_vars['target_'+sys_name2] = (0, len(sup_target_list2))
	
	#Envrionment init value
	#env_init = {'target_{sys_name} = {last_value}'.format(sys_name=sys_name, last_value=len(sup_target_list))}
	env_init = set()
	env_init |= {'target_{sys_name} = {last_value}'.format(sys_name=sys_name, last_value=len(sup_target_list))}
	env_init |= {'target_{sys_name} = {last_value}'.format(sys_name=sys_name2, last_value=len(sup_target_list2))}
	...
	env_prog |= {"! (target_{sys_name} = {last_value})".format(sys_name=sys_name2 , last_value=len(sup_target_list2))}
	...
	env_safe |= {'(target_{sys_name} ={last_value}) -> X (!(target_{sys_name} = {last_value}))'.format(sys_name=sys_name2, last_value=len(sup_target_list2))}
	...
	for elem in range(len(sup_target_list2)):
		env_safe |= {'(target_{sys_name} ={elem}) -> X ((target_{sys_name} = {elem}) || (target_{sys_name} = {last_value}))'.format(sys_name=sys_name2,elem=elem,last_value=len(sup_target_list2))}

	#system variable
	sys_vars = {}
	sys_vars[sys_name] = (0 , (row_number * col_number) -1)
	sys_vars[sys_name2] = (0 , (row_number * col_number) -1)
	#sys_vars['follow_target'] = (0,1)
	sys_vars['follow_'+sys_name] = (0,1)
	sys_vars['follow_'+sys_name2] = (0,1)
	sys_vars['stage_'+sys_name] = (0, len(list_target))
	sys_vars['stage_'+sys_name2] = (0, len(list_target2))

	#system init variable
	sys_init = set()
	sys_init |= {"(stage_{sys_name} = {init_value})".format(sys_name=sys_name , init_value=0)}
	sys_init |= {"(stage_{sys_name} = {init_value})".format(sys_name=sys_name2 , init_value=0)}
	#sys_init |= {'follow_target = 0'}
	sys_init |= {'follow_{sys_name} = 1'.format(sys_name=sys_name)}
	sys_init |= {'follow_{sys_name} = 1'.format(sys_name=sys_name2)}
	sys_init |= {sys_name + ' = '+ str(init_state)}
	sys_init |= {sys_name2 + ' = '+ str(init_state2)}

	#System prog
	sys_prog = set()
	sys_prog |= {"(stage_{sys_name} = {last_value})".format(sys_name=sys_name,last_value=len(list_target))}
	sys_prog |= {"(stage_{sys_name} = {last_value})".format(sys_name=sys_name2,last_value=len(list_target2))}
	sys_prog |= {'(follow_{sys_name} = 1) && (follow_{sys_name2} = 1)'.format(sys_name=sys_name , sys_name2=sys_name2)}
	...
	for elem in range(len(list_target2)):
		sys_safe |= {'(stage_{sys_name} = {value}) ->  X ((stage_{sys_name} = {value}) || (stage_{sys_name} = {value_1}))'.format(sys_name=sys_name2,value=elem, value_1=elem+1)}

	sys_safe |= {'(stage_{sys_name} = {last_value}) ->  X ( (stage_{sys_name} = 0) )'.format(sys_name=sys_name, last_value=len(list_target))}
	sys_safe |= {'(stage_{sys_name} = {last_value}) ->  X ( (stage_{sys_name} = 0) )'.format(sys_name=sys_name2, last_value=len(list_target2))}

	sys_safe |= {' (target_{sys_name} = {last_value}) ->  X (follow_{sys_name} = 0)'.format(sys_name=sys_name,last_value=len(sup_target_list))}
	sys_safe |= {' (target_{sys_name} = {last_value}) ->  X (follow_{sys_name} = 0)'.format(sys_name=sys_name2,last_value=len(sup_target_list2))}
	...
	sys_safe |= allow_moves(row_number , col_number , sys_name , sys_name2, list_obstacles)
	sys_safe |= handle_static_obstacles(list_obstacles , sys_name ,  sys_name2)
	sys_safe |= handle_target_point(row_number , col_number , list_target , sys_name , sup_target_list)
	sys_safe |= handle_target_point(row_number , col_number , list_target2 , sys_name2 , sup_target_list2)
	#sys_safe |= no_collision(row_number , col_number , sys_name , sys_name2)
...
class AgentController:
	def __init__(self , init_state , init_state2, file_name, row_number , col_number , list_target ,list_target2 , sys_name , sys_name2, sup_target_list , sup_target_list2, list_obstacles ):
		self.file_name = file_name
		self.row_number = row_number
		self.col_number = col_number
		self.list_target = list_target
		self.list_target2 = list_target2
		self.sys_name = sys_name
		self.sup_target_list = sup_target_list
		self.sup_target_list2 = sup_target_list2
		self.list_obstacles = list_obstacles
		self.init_state = init_state
		self.init_state2 = init_state2
		self.sys_name2 = sys_name2

	def createController(self):
		solve_problem(self.init_state, self.init_state2,self.file_name, self.row_number , self.col_number , self.list_target ,self.list_target2, self.sys_name, self.sys_name2, self.sup_target_list ,self.sup_target_list2 , self.list_obstacles)
...
if __name__ == "__main__":
	...
	sys_name = 'Quad9'
	sys_name2 = 'Quad10'
	sup_target_list = [19 , 6]
	sup_target_list2 = [23 , 9]
	list_obstacles =[8,16]

	agent = AgentController(4, 20 , file_name , row_number , col_number , list_target , list_target2, sys_name , sys_name2 , sup_target_list, sup_target_list2 , list_obstacles)
	#agent.createController()
	controller = agent.getController()

	for elem in (range(50)):
		target1 = int(raw_input('target1 ind : '))
		target2 = int(raw_input('target2 ind : '))
		print controller.move(target1 , target2)
